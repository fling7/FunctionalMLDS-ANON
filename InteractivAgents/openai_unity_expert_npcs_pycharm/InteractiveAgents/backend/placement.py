...
def _agent_candidates_from_preview(preview_agents: Any) -> Dict[str, Dict[str, Any]]:
    ...
    for entry in preview_agents:
        ...
        raw_forward = entry.get("forward")
        if isinstance(raw_forward, dict):
            try:
                candidate["forward"] = {
                    "x": float(raw_forward.get("x", 0.0)),
                    "y": float(raw_forward.get("y", 0.0)),
                    "z": float(raw_forward.get("z", 0.0)),
                }
            ...
            placement.update(
                {
                    ...
                    "position": {
                        "x": float(pos.get("x", 0.0)) if abs(resolved[0] - start[0]) < 1e-9 else round(resolved[0], 3),
                        "y": float(pos.get("y", 0.0)),
                        "z": float(pos.get("z", 0.0)) if abs(resolved[2] - start[2]) < 1e-9 else round(resolved[2], 3),
                    },
                    "forward": forward,
                }
            )
            agent_placements.append(placement)
        else:
